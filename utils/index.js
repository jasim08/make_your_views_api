...
module.exports = {
  OTPGenerator: function () {
    return crypto.randomBytes(8).toJSON().data.join("").substring(0, 4);
  },
  ...
  cleanSensitiveData: function (data) {
    data.password = data.password ? delete data.password : data;
    return data;
  },
  ...
  checkEmailOrMobileValidity: function checkEmailOrMobile(input) {
    if (
      /^[0-9]{10}$/.test(input) ||
      /^[a-zA-Z0-9+_.-]+@[a-zA-Z0-9.-]+$/.test(input)
    ) {
      return true;
    }
    return false;
  ...
  generateUniqueFileName: () => {
    return "myviews." + crypto.randomUUID();
  },
};
